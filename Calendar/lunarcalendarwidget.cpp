...
void LunarCalendarWidget::initWidget()
{
    ...
    //新建日程
    QPushButton *btnEvent = new QPushButton;
    btnEvent->setObjectName("btnEvent");
...
//    AM_RadioButton->setText("上午");
    ...
    //AMorPM_combox = new QComboBox;
    //AMorPM_combox->addItem("上午");
    //AMorPM_combox->addItem("下午");

//    //下午
//    QRadioButton *PM_RadioButton = new QRadioButton;
//    PM_RadioButton->setText("下午");
    ...
    //layoutTop->addWidget(AMorPM_combox);
...
void LunarCalendarWidget::yearChanged(const int index)
{
...
void LunarCalendarWidget::monthChanged(const int index)
{
    //如果是单击按钮切换的日期变动则不需要触发
    if (btnClick) {
        return;
...
void LunarCalendarWidget::dayChanged(const QDate &date)
{
    ...
    int month = date.month();
    int day = date.day();
//    qDebug()<<"["<<__FILE__<<"]"<<__LINE__<<__FUNCTION__<<" "<<day;
    int week = LunarCalendarInfo::Instance()->getFirstDayOfWeek(year, month);
...
void LunarCalendarWidget::updateCalendarEvents() const
{
 ...
 * @brief 获取当前选择日期
 */
void LunarCalendarWidget::retCurrData()
{
    QString currData = date.toString("yyyy-MM-dd");
    //currData.append("-");
    //currData.append(AMorPM_combox->currentText());
    emit signalCurrData(currData);
    this->close();
}
