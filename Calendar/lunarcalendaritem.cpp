...
void LunarCalendarItem::drawDay(QPainter *painter)
{
    ...
    if (showLunar) {
        ...
        QRect dayRect = QRect(0, 0, width/2, height / 3.5);
...
void LunarCalendarItem::drawLunar(QPainter *painter)
{
    ...
    QRect lunarRect(width/2, 0, width/2, height / 3);
...
// 渲染事件到日历单元格的方法
void LunarCalendarItem::drawEvents(QPainter *painter) {
    painter->save();

    QVector<TodoEvent> events = storage.getAllEvents();
    QFont font;
    font.setPixelSize(11); // 根据需要调整字体大小
    painter->setFont(font);

    QRectF eventRect = rect().adjusted(5, height() / 4, -5, -5); // 调整填充，将第一个事件名称放置在格子的1/5高度位置
    int maxEvents = 3;
    int eventCount = 0;
    ...
    for (const TodoEvent &event : events) {
        ...
        if (event.startTime.date() <= date && event.endTime.date() >= date) {
            // 根据事件紧急程度选择颜色
            QColor textColor;
            QColor customColor(255, 165, 0); // 使用 RGB 值创建橙色
            if (event.priority == "Low") {
                textColor = Qt::green; // 低优先级，绿色
            } else if (event.priority == "Middle") {
                textColor = customColor; // 中等优先级，橙色
            } else if (event.priority == "High") {
                textColor = Qt::red; // 高优先级，红色
            } else {
                textColor = Qt::black; // 默认为黑色
            }

            if (eventCount < maxEvents) {
                // 计算事件文本的大小
                QRectF textRect = QRectF(eventRect.topLeft(), QSizeF(eventRect.width(), QFontMetrics(font).height()));
                // 在事件矩形内居中绘制事件文本，使用指定的颜色
                painter->setPen(textColor);
                painter->drawText(textRect, Qt::AlignCenter, event.name);
                // 调整事件矩形的位置，使下一个事件在下方显示
                eventRect.adjust(0, textRect.height(), 0, 0);
                ++eventCount;
            } else {
                // 如果超过最大事件数，绘制省略号并退出循环
                QFont ellipsisFont = font;
                ellipsisFont.setPointSize(13); // 调整省略号字体大小
                painter->setFont(ellipsisFont);
                QRectF ellipsisRect = QRectF(eventRect.topLeft().x(), eventRect.topLeft().y() - QFontMetrics(ellipsisFont).height() / 2, eventRect.width(), QFontMetrics(ellipsisFont).height());
                painter->setPen(Qt::black); // 省略号默认为黑色
                painter->drawText(ellipsisRect, Qt::AlignCenter, "...");
                break;
            }
        }
    }
